...
class SelectCostumerForm( forms.ModelForm ):
    def __init__(self, *args, **kwargs):
        super().__init__( *args, **kwargs )
        self.fields['costumer'].queryset = Costumer.objects.filter( is_deleted=False )
        self.fields['costumer'].widget.attrs.update( {'class': 'textinput form-control'} )
...
class AgentForm( forms.ModelForm ):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['phone'].widget.attrs.update(
            {'class': 'textinput form-control', 'maxlength': '12', 'pattern': '[0-9]{12}', 'title': 'Numbers only'} )
...
class CostumerForm( forms.ModelForm ):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['address'].widget.attrs.update(
            {'class': 'textinput form-control', 'maxlength': '200', 'pattern': '[A-Z0-9]{200}',
             'title': 'Address Format Required'} )
        self.fields['email'].widget.attrs.update( {'class': 'textinput form-control'} )
        self.fields['nipt'].widget.attrs.update(
            {'class': 'textinput form-control', 'maxlength': '10', 'pattern': '[A-Z0-9]{10}',
             'title': 'Nipt Format Required'} )
        self.fields['agent'].widget.attrs.update(
            {'class': 'textinput form-control', } )

    class Meta:
        model = Costumer
        fields = ['name', 'phone', 'address', 'email', 'nipt', 'agent']
        widgets = {
            'address': forms.Textarea(
                attrs={
                    'class': 'textinput form-control',
...
class SupplierForm( forms.ModelForm ):
    ...
    class Meta:
        model = Supplier
        fields = ['name', 'phone', 'address', 'email', 'nipt']
        widgets = {
            'address': forms.Textarea(
                attrs={
                    'class': 'textinput form-control',
...
class SaleForm( forms.ModelForm ):
    def __init__(self, *args, **kwargs):
        super().__init__( *args, **kwargs )
        self.fields['costumer'].widget.attrs.update(
            {'class': 'textinput form-control', 'pattern': '[a-zA-Z\s]{1,50}', 'title': 'Alphabets and Spaces only',
             'required': 'true'} )
        # self.fields['phone'].widget.attrs.update(
        #     {'class': 'textinput form-control', 'maxlength': '10', 'pattern': '[0-9]{10}', 'title': 'Numbers only',
        ...
        self.fields['costumer'].widget.attrs.update( {'class': 'textinput form-control', 'required': 'true'} )
